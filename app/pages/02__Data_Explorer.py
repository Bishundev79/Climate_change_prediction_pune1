...
if df is not None:
    ...
    with s1:
        st.markdown(gradient_card("Mean", f"{df_filtered[variable].mean():.2f}", variant="gc-purple"), unsafe_allow_html=True)
    with s2:
        st.markdown(gradient_card("Std Dev", f"{df_filtered[variable].std():.2f}", variant="gc-pink"), unsafe_allow_html=True)
    with s3:
        st.markdown(gradient_card("Min", f"{df_filtered[variable].min():.2f}", variant="gc-blue"), unsafe_allow_html=True)
    with s4:
        st.markdown(gradient_card("Max", f"{df_filtered[variable].max():.2f}", variant="gc-orange"), unsafe_allow_html=True)
