...
if __name__ == '__main__':
    ...
    try:
        ...
        # ML ensemble (both models have same test set)
        if len(ml_predictions) >= 2:
            ml_ensemble_pred = calculate_ensemble(ml_y_test, ml_predictions)
            ml_ensemble_metrics = Evaluator.evaluate(ml_y_test, ml_ensemble_pred, "ML Ensemble")
            all_results['ML Ensemble'] = ml_ensemble_metrics
        
        # DL ensemble (if DL models were trained)
        if TENSORFLOW_AVAILABLE and dl_results:
            all_results.update(dl_results)
            if len(dl_predictions) >= 2:
                dl_ensemble_pred = calculate_ensemble(dl_y_test, dl_predictions)
                dl_ensemble_metrics = Evaluator.evaluate(dl_y_test, dl_ensemble_pred, "DL Ensemble")
                all_results['DL Ensemble'] = dl_ensemble_metrics
